...
def test_load_tier_configs_both_providers(tier_yaml: Path) -> None:
    """Both providers resolve when env vars are set."""
    ...
    assert t1.primary.api_key == "tok-123"
    assert t1.fallback is not None
    assert t1.fallback.name == "google"
    assert t1.fallback.api_key == "gem-456"


def test_load_tier_configs_primary_missing(tier_yaml: Path) -> None:
    """Primary skipped when its env var is unset; fallback still resolves."""
    env = {"GEMINI_API_KEY": "gem-789"}
    with patch.dict("os.environ", env, clear=False):
        # Ensure OPENCLAW_OAUTH_TOKEN is NOT set
        with patch.dict("os.environ", {"OPENCLAW_OAUTH_TOKEN": ""}, clear=False):
            tiers = load_tier_configs(tier_yaml)

    t1 = tiers["tier1_heavy_reasoning"]
    assert t1.primary is None
    assert t1.fallback is not None
    assert t1.fallback.name == "google"
...
def test_client_uses_primary_first() -> None:
    """Primary is called first; fallback is not touched on success."""
    primary = _make_provider("primary")
    fallback = _make_provider("fallback")
    tier = TierConfig("test", primary=primary, fallback=fallback)
...
def test_client_falls_back_on_primary_failure() -> None:
    """When primary raises, fallback is tried."""
    primary = _make_provider("primary")
    fallback = _make_provider("fallback")
    tier = TierConfig("test", primary=primary, fallback=fallback)
    client = LLMClient(tier=tier)
...
def test_client_raises_when_all_fail() -> None:
    """RuntimeError when both primary and fallback fail."""
    primary = _make_provider("primary")
    fallback = _make_provider("fallback")
    tier = TierConfig("test", primary=primary, fallback=fallback)
    client = LLMClient(tier=tier)
...
def test_client_raises_when_no_providers() -> None:
    ...
    tier = TierConfig("empty", primary=None, fallback=None)
    client = LLMClient(tier=tier)
...
def test_client_only_fallback_when_primary_missing() -> None:
    """When only fallback has credentials, it's used directly."""
    fallback = _make_provider("gemini")
    tier = TierConfig("test", primary=None, fallback=fallback)
    client = LLMClient(tier=tier)
...
def test_master_bridge_calls_llm() -> None:
    ...
    fake_client = LLMClient(
        tier=TierConfig("test", primary=_make_provider("test"))
    )
